...
_, ax1 = plot_scan_results(
    f"{data_dir}/annih_swave_ee.dat", output_file=None, xc=1, yc=0,
...
_, ax2 = plot_scan_results(
    f"{data_dir}/annih_pwave_Tkd_1e+00MeV_ee.dat", output_file=None, xc=1, yc=0,
...
_, ax4 = plot_scan_results(
    f"{data_dir}/decay_mphi_5e+01MeV_aa.dat", output_file=None, fix_helium=True, xc=1, yc=0,
    title=tex_title(mphi=50, temp0=10, braa=1), labels=tex_labels("tau", "n0a")
)

# Add labels for the different abundances
ax4.text(4.0,  -9.7, r"D/$^1$H low"               , color="0.6"           , fontsize=16)
ax4.text(7.5, -11.1, r"$^3$He/D high"             , color="mediumseagreen", fontsize=16)
